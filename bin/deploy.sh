...
args=(-azvP --delete --exclude=node_modules --exclude=.idea --exclude=data --exclude=uploads --exclude=static/_posts)
...
if [ "$result" == "exists" ]; then
    ...
    do
      echo ""
      date
      rsync ${dry[@]} ${args[@]} ./bin ${user}@${host}:www/${name}/${project}/bin
