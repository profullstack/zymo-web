...
export default {
    ...
    async post(request) {
        ...
        if (amount > affiliate.balance) {
            return view("admin/affiliates/Payout.svelte", { affiliate, error: "Amount cannot be larger than affiliate balance" })
        }
        const payoutMethod = affiliate.payoutMethods.find((method) => method.id == payoutMethodId);

        if (!payoutMethod) {
            return view("admin/affiliates/Payout.svelte", { affiliate, error: "Invalid payout method" })
        }

        const payout = await Payout.create(amount, payoutMethod.method, payoutMethod.details, affiliate.userId)
        ...
        return view("admin/affiliates/Payout.svelte", { affiliate, message: "payout created successfully" })
