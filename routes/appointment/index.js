...
const addEventToMainCalendar = async (
	summary,
	description,
	startTime,
	endTime,
...
const addEventToUserCalendar = async (
	summary,
	description,
	startTime,
	endTime,
...
const getAppointment = async (
	...
	if (mainCalendarEventId && userCalendarEventId) {
		...
		let [mainCalendarEvent, userCalendarEvent] = await Promise.all([
			mainCalendar.events.get({
				calendarId: GOOGLE_CALENDAR_ID,
				eventId: mainCalendarEventId
			}),
		...
		if (
			userCalendarEvent?.status === 'cancelled' &&
			mainCalendarEvent?.status !== 'cancelled'
		) {
			...
			if (dbAppointment) {
				try {
					appointment = await getAppointment(
						dbAppointment.id,
						Appointment,
						dbAppointment.mainCalendarEventId,
						dbAppointment.userCalendarEventId,
						refreshToken
					);

					if (appointment == 'error') {
						return view('Appointment.svelte', {
							authorized,
							error: 'An error occured while fetching appointment, If the issue persists try reauthenticating google calendar'
						});
					}
				} catch (e) {}
			}
		...
		try {
			...
			if (!authorized) {
				return view('Appointment.svelte', {
					authorized,
					GOOGLE_CLIENT_ID,
					error: 'Please authorize google calendar'
				});
			}
			...
			if (mainCalendarConflicts) {
				return view('Appointment.svelte', {
					authorized,
					error: 'Sorry, We are unavailable at that time'
				});
			}

			const userCalendarConflicts = await checkUserCalendarConflicts(
				refreshToken,
				startTime,
				endTime
			);
			...
			if (userCalendarConflicts) {
				return view('Appointment.svelte', {
					authorized,
					error: 'You already have an event set at that time'
				});
			}

			const mainCalendarEvent = await addEventToMainCalendar(
				summary,
				description,
				startTime,
				endTime,
			...
			const userCalendarEvent = await addEventToUserCalendar(
				summary,
				description,
				startTime,
				endTime,
			...
			return view('Appointment.svelte', {
				authorized,
				error: 'An error occured, If the issue persists try reauthenticating google calendar'
			});
		}
	}
};
